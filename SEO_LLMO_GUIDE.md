...
export const metadata: Metadata = {
  ...
  // === OpenGraph (SNS共有時) ===
  openGraph: {
    title: `ページタイトル${site.seo.titleSuffix}`,
    description: "ページの説明文",
    url: `${site.seo.siteUrl}/page-path`,
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  // site.news などからデータを取得
  const item = site.news.find((n) => n.slug === slug);
  ...
  const description = `${item.title} - ${company.name}からのお知らせ`;
